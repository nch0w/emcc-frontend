...
async function run() {
  ...
  Object.keys(coaches).forEach(async (coachID, i) => {
    ...
    if (coachID !== "alan.bu2017@gmail.com") return;
    console.log(coachID);
    try {
      //await compDayMail(coachID, coaches[coachID]);
      await compDayMail("alan.bu2016@gmail.com", coaches[coachID]);
    } catch (err) {
      console.log(err);
    }
    // await compDayMail(coachID, coaches[coachID]);
