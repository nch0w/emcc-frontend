...
const Dashboard = () => {
    ...
    const handleUpdateCoachInfo = () => {
        // validate form
        if (name.length === 0 || phone.length === 0 || email.length === 0 || mail.length === 0) {
            setStatus(dashboardStatus.InvalidCoachInfo);
            setError('All fields are required.')
            return;
        }
        // submit form
        axios.post(emccServerUrl + '/update-coach-info', {
            name: name,
            phone: phone,
            email: email,
            mail: mail
    ...
    const validateTeam = (numTeams, student1, student2, student3, student4) => {
        if (numTeams >= user.coachInfo.teamLimit) {
            setError('Team limit reached. Please contact the Exeter Math Club email to request more teams. Requests will be evaluated on a case-by-case basis.');
            setStatus(dashboardStatus.InvalidCompetitor);
            return false;
        }
        if ((isEmpty(student1) && !isEmpty(student2)) ||
        ...
        for (let i = 0; i < validStudents.length; i++) {
            for (let j = 0; j < validStudents.length; j++) {
                if (i !== j && validStudents[i] === validStudents[j]) {
                    setError('Students on the same team must have different names, to avoid problems during grading.');
                    setStatus(dashboardStatus.InvalidCompetitor);
                    return false;
                }
            }
    ...
    const validateIndividual = (numIndivs, studentName) => {
        if (numIndivs >= user.coachInfo.indivLimit) {
            setError('Individual limit reached. Please contact the Exeter Math Club email to request more individuals. Requests will be evaluated on a case-by-case basis.');
            setStatus(dashboardStatus.InvalidCompetitor);
            return false;
        }
    ...
    const renderTab = () => {
        switch (activeTab) {
                ...
                return (
                    <Box>
                            ...
                            editable={{
                                onRowAdd: newData =>
                                    new Promise((resolve, reject) => {
                                        if (validateTeam(numTeams, newData.student1, newData.student2, newData.student3, newData.student4)) {
                                            ...
                                            axios.post(emccServerUrl + '/add-team', newData, { timeout: 5000 })
                                                .then(response => {
                                ...
                                onRowUpdate: (newData, oldData) =>
                                    new Promise((resolve, reject) => {
                                        if (validateTeam(numTeams, newData.student1, newData.student2, newData.student3, newData.student4)) {
                                            axios.post(emccServerUrl + '/update-team', newData)
                                                .then(_response => {
                                ...
                                onRowDelete: oldData =>
                                    new Promise((resolve, _reject) => {
                                        axios.post(emccServerUrl + '/delete-team', oldData)
                                            .then(_response => {
                                                const index = oldData.tableData.id;
                        ...
                        <MaterialTable
                            ...
                            icons={tableIcons}
                            columns={[
                                { title: 'Student Name', field: 'student' }
                            ]}
                            ...
                            editable={{
                                onRowAdd: newData =>
                                    new Promise((resolve, reject) => {
                                        if (validateIndividual(numIndivs, newData.student)) {
                                            ...
                                            console.log('got to onRowAdd');
                                            axios.post(emccServerUrl + '/add-indiv', newData, { timeout: 5000 })
                                                .then(response => {
                                ...
                                onRowUpdate: (newData, oldData) =>
                                    new Promise((resolve, reject) => {
                                        if (validateIndividual(numIndivs, newData.student)) {
                                            axios.post(emccServerUrl + '/update-indiv', newData)
                                                .then(_response => {
                                                    const index = oldData.tableData.id;
                                ...
                                onRowDelete: oldData =>
                                    new Promise((resolve, _reject) => {
                                        axios.post(emccServerUrl + '/delete-indiv', oldData)
                                            .then(_response => {
                                                const index = oldData.tableData.id;
            ...
            case 'view-coach-info':
                return (
                    <Box>
                        <TextField required id='coach-name' label='Name' value={name} onChange={event => setName(event.target.value)} variant='outlined' />
                        <br /><br />
                        <TextField required id='coach-phone' label='Phone Number' value={phone} onChange={event => setPhone(event.target.value)} type='tel' variant='outlined' />
                        <br /><br />
                        <TextField required id='coach-email' label='Email Address' value={email} onChange={event => setEmail(event.target.value)} variant='outlined' />
                        <br /><br />
                        <TextField required id='coach-mail' label='Mailing Address' value={mail} onChange={event => setMail(event.target.value)} variant='outlined' />
                        <br /><br />
                        <Button variant='outlined' onClick={() => handleUpdateCoachInfo()}>Update Info</Button>
                        <br /><br />
                        {renderMessage()}
                    </Box>
                );
    ...
    return (
        <Container>
            ...
            <Paper elevation={3}>
                ...
                >
                    <Tab value='view-competitors' label='View and Edit Competitors (Teams and Individuals)' />
